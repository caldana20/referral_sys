...
Local GCS for dev (recommended):
- Create a GCS bucket (e.g. `tenant-media`) in your GCP project.
- Create a service account key JSON with access to the bucket.
- Set envs: `GCS_BUCKET_MEDIA=tenant-media`, `GCS_PUBLIC_BASE_URL=https://storage.googleapis.com/tenant-media`, `GCS_PROJECT_ID=<project-id>`, `GCS_KEYFILE=/path/to/key.json`, `GCS_SIGNED_URL_EXPIRY=3600`.
